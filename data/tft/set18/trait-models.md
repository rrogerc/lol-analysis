...
## Update, September 22, 2026 (live patch 18.2b)

- **Caustic is per target and timed.** Composition scoring used to turn Kog'Maw's 30% Sunder/Shred into standing coverage of every target, for every ally, from the first second. Now a timed on-hit reduction (Caustic, Last Whisper, Void Staff, a driver's own) lands on the target its holder hits, lasts its duration there, and every ally hitting that same generic target shares it while it runs. Auras (Evenshroud, Ionic Spark) stay standing coverage. The items had to move with Caustic: the 124 published boards without Kog'Maw were exactly the ones carrying Last Whisper, Void Staff or an aura in his place.
- **Solar's true-damage share reads its row.** At five 3★ units the resolver converted a hardcoded half of the bonus; 18.2b's `Threshold2TrueDamageConversion` is 0.4.
- **Summoner gives Azir his own multiplier.** From 18.2b on his soldiers read `AzirDamageMult` (+20%/+30%) while Mama Beak keeps `DamageMult` (+45%/+67.5%); earlier patches fall back to `DamageMult`, as their trait text does.

The numbers quoted in the table below are 18.1d's; the engine reads the live patch's rows.
...
| Attuned | Central reward missing | Alune's full-moon damage exists (every fifth cast since 2026-09-20); team phase buffs contribute zero. | Needs verified initial moon phase and cast-driven team windows: 7% durability at/below half moon, otherwise 7% amp. Do not apply both permanently or guess equal uptime. |
...
| Caustic | Core combat modeled | Kog'Maw's damage applies 30% Sunder/Shred for 4s to the target it lands on; allies hitting that generic target share it while it runs (2026-09-22). | Nothing reduces a target before his first hit or after the debuff runs out, so provider death is covered. Allies share a target by generic index, the model's usual abstraction. |
...
| Solar | Partial | All allies receive shield and bonus damage scaled by actual unique three-star count; at 3 three-stars the existing AS/resists apply. At 5 three-stars, the `Threshold2TrueDamageConversion` share of the existing bonus (half on 18.1d, 40% on 18.2b; a hardcoded half until 2026-09-22) becomes true damage in ordinary and bridged native paths. No bonus amount is added by conversion. | Eight-three-star recurring four-star ascension remains absent. Existing Solar damage-origin/proc classification is retained, including no recursive bonus procs and exclusion of raw/true source damage. Generic geometry and action timing remain. |
...
| Summoner | Partial | At 2/3: Yorick summon HP ×1.30/1.45; Mama Beak damage ×1.45/1.675; Azir the same on 18.1d, his own ×1.2/1.3 from 18.2b (`AzirDamageMult`, 2026-09-22); Zyra +4/+6 attacks. | **Removed unsupported overcredit:** orphaned ExtraSummons/SummonPower mappings. Azir and Zyra keep their base summon counts. Plant attack cadence remains approximate; no live script trace was obtained. |
