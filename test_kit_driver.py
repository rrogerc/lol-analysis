...
class TestReferenceDriver(unittest.TestCase):
    ...
    def test_opening(self):
        # R's swing lands after its 0.25 s cast: 250 at rank 3, no AP
        self.assertNotIn("R", self.sim(0.24)["breakdown"])
        self.assertAlmostEqual(self.sim(0.25)["breakdown"]["R"], 250.0)
        # Leap Strike at t=0: W > Q > E at level 16 is W 5, Q 5, E 3, so 225, no bonus AD
        self.assertAlmostEqual(self.sim(0.0)["breakdown"]["Q"], 225.0)

    def test_counter_strike_recast(self):
        # cast at 0, recast a second later: rank 3's 100 + 4% of the dummy's 100,000
        self.assertNotIn("E", self.sim(0.99)["breakdown"])
        self.assertAlmostEqual(self.sim(1.0)["breakdown"]["E"], 100.0 + 4000.0)
        # its 13 s cooldown starts at the recast: the next one is cast at 14, lands at 15
        self.assertAlmostEqual(self.sim(14.99)["breakdown"]["E"], 4100.0)
        self.assertAlmostEqual(self.sim(15.0)["breakdown"]["E"], 2 * 4100.0)
