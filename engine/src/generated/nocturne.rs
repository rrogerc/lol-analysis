...
//! (Q) is cast on cooldown, has a real 0.25s cast time, and its trail grants
//! a temporary bonus-AD steroid to attacks (and, live, to P and R). Shroud
//! of Darkness (W) contributes only its always-on passive attack speed.
//! Unspeakable Horror (E) is cast on cooldown and ticks its DoT in full
//! since the tether never breaks. Paranoia (R) opens the fight, recasts
//! almost immediately for its dash damage, and repeats its cast/recast
//! cycle off cooldown. Every cast, even one with no cast time, waits for a
//! cast already in progress via the shared `busy_until` state.
    ...
    fn events(&self, e: &Engine, out: &mut Events) -> usize {
        ...
        if self.ranks.r > 0 {
            if self.s.r_recast_at != INF {
                ...
                out[n] = (self.castable_at(e, self.s.r_ready), Kind::Ev(EV_R_INITIATE));
            }
            n += 1;
        }
        n
