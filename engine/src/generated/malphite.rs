...
//! attack for 5 s (including that one) also gets the cone. Thunderclap's own
//! passive bonus armor is assumed permanently tripled (Granite Shield never
//! breaks against a dummy that never attacks) and read recursively, feeding
//! its own on-hit/cone and Ground Slam's armor ratio.
