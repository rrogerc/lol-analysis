...
//! and mark proc while it is active. Casts go one at a time: R's and Q's
//! real cast times keep K'Sante busy (no other cast, no attack) until they
//! end; Path Maker's charge has no cast time of its own but still waits for
//! any cast already in progress.
