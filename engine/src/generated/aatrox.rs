...
//! used the instant it is available. Casts go one at a time: a single
//! busy_until timer holds off every other cast and attack for a cast's
//! duration (0.6s for Q, 0.25s for W and R; E has none).
...
pub struct GenDriver {
    ...
    /// cooldown, the static gap between casts and its cast time.
    q_base: f64,
    q_adratio: f64,
    q_ramp_bonus: f64,
    q_sweet_bonus: f64,
    ...
    /// Umbral Dash.
    e_cd: f64,
    /// World Ender: bonus AD as a fraction of total AD, its duration and
    /// its cast time.
    r_ad_ratio: f64,
    r_active_s: f64,
    r_cast_s: f64,
...
impl Driver for GenDriver {
    ...
    fn events(&self, e: &Engine, out: &mut Events) -> usize {
        ...
        if self.ranks.e > 0 {
            out[n] = (self.castable_at(e, self.s.e_ready), Kind::Ev(EV_E_READY));
            n += 1;
        }
        n
    }
